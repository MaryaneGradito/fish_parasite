...
dat_rep<-data.frame(
  ...
  "Repeatability"=c(R_bold_mean,R_exp_mean,R_act_mean
                    ,R_C_bold,R_exp_C_mean,R_act_C,R_E_bold,R_exp_E_mean,R_act_E,0.0464,0.417,0.314), 
  "Lower_CI" = c(R_Blb_bold,R_Blb_exp,R_Blb_act,R_Blb_bold_C,R_Blb_exp_C,R_Blb_act_C,R_Blb_bold_E,R_Blb_exp_E,R_Blb_act_E,0,0,0),
  "Higher_CI" = c(R_Bub_bold,R_Bub_exp,R_Bub_act,R_Bub_bold_C,R_Bub_exp_C,R_Bub_act_C,R_Bub_bold_E,R_Bub_exp_E,R_Bub_act_E,0.555,0.782,0.752))
...
```{r}
#| label: fig-fig5
#| fig-cap: Contrast in the mean responses and the 95% confidence intervals of shyness (A), exploration (B) and activity (C) of pumpkinseed sunfish (Lepomis gibbosus) between the uninfected (N=72) and experimentally infected group (N=60)
#| out-width: 100%
#| fig-width: 8
#| fig-height: 5
...
#Exploration confidence intervals
  #Control
blb_exp_int <- round(quantile(int_exp_c_num, c(0.025, 0.975))[1], 3)
bub_exp_int <- round(quantile(int_exp_c_num, c(0.025, 0.975))[2], 3)
  #Experimental
blb_exp_T <- round(quantile(int_exp_e, c(0.025, 0.975))[1], 3)
bub_exp_T <- round(quantile(int_exp_e, c(0.025, 0.975))[2], 3)
...
#boldness
m1<-
ggplot(all_data, aes(x = trt, y = z_log_boldness, group = trt, color = trt)) +
...
theme(legend.position = "none",
 legend.title = element_blank()) +
geom_segment(aes(x = 1, xend = 2, y = 1, yend = 1),color = "black") +
annotate("text",  x = 1.5, y = 1, label = TeX(paste0("Contrast = 0.43, $pMCMC_{\\alpha = 0.05}$ = 0.0015**")), size =2.5)

#activity
dat3<-dat_model %>% 
  filter(trait == "logactivity")

m3<-ggplot(all_data, aes(x = trt, y = z_log_activity, group = trt, col = trt)) +
geom_violin(aes(), trim = FALSE) +
geom_jitter(aes(colour = trt), shape=16, position=position_jitter(0.1)) +
scale_color_manual(labels = c("Uninfected", "Infected"), values = c("#472a7a", "#1f978b"))+
...
annotate("text",  x = 1.5, y = 4, label = TeX(paste0("Contrast = -0.26, $pMCMC_{\\alpha = 0.05}$ = 0.0666")), size =2.5)
 ...
 ggsave("./output/figures/sandra.png", plot = m3, width = 10, height = 13, units = "cm")
 act_sandra <- image_read("./output/figures/sandra.png") 
 
 ggsave("./output/figures/fig5.png", plot = m, width = 28, height = 13, units = "cm")
...
k1<-ggplot(data= new_data)+
  ...
  xlab("Change in black spots density") + ylab("Slope of change in activity") + theme_classic()+geom_vline(xintercept=0, color = "red2", linetype = 2)+
  labs(color = "Change in \n body condition") + scale_colour_gradient(low = "#112D72",high = "#C7D1ED")

ggsave("./output/figures/fig9.png", plot = k1, width = 30, height = 20, units = "cm")
