...
#plot
 x1<- ggplot(data=dat_C)+
  geom_point(mapping = aes(x = z_bc, y = log_activity), shape = 21,color = "pink3")+
 geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_act_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "pink3", size=1) +
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "pink3", size=0.8)+
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "pink3", size=0.8)+
   theme_classic() +
    ...
    theme(legend.position="none") + labs(x = TeX("Body condition $(g/cm^{3})$"), y = TeX("$Activity_{log_{2}}$ (cm)"), title = "")+
      annotate("text", x = 0, y = 2.6, label = TeX(paste0("$beta_{bc}$ = ", " 0.21 ", " 95%CI: 0.04, 0.39**")), size =3, color = "pink4") 
...
x2<- ggplot(data=dat_parasite)+
  geom_point(mapping = aes(x = z_bc, y = log_activity), shape = 21,color = "pink3")+
 geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_act_bc), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "pink3", size=1) +
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "pink3", size=0.8)+
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "pink3", size=0.8)+
    geom_point(mapping = aes(x = z_pl, y = log_activity), shape = 21,color = "aquamarine3")+
 geom_smooth(data = act_pl_dat, ggplot2::aes(x = z_pl, y = summ_act_pl), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "aquamarine3", size=1) +
  geom_smooth(data = act_pl_dat, ggplot2::aes(x = z_pl, y = summ_actL95_pl), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
  geom_smooth(data = act_pl_dat, ggplot2::aes(x = z_pl, y = summ_actU95_pl), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
   theme_classic() +
  draw_image("parasite.png", x= 2,y = 2, scale = 1.2)+
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition $(g/cm^{3})$"), y = TeX("$Activity_{log_{2}}$ (cm)"), title = "")+
  annotate("text", x = 3, y = -1.5, label = TeX(paste0("Parasite load")), size =3.5, color = "aquamarine4", fontface = "bold") +
  annotate("text", x = -2, y = 0, label = TeX(paste0("Body condition")), size =3.5, color = "pink4") +
      annotate("text", x = 0, y = 2.5, label = TeX(paste0("$beta_{pl}$ = ", " -0.18 ", " 95%CI: -0.37, 0.01")), size =3, color = "aquamarine4") + 
  annotate("text", x = 0, y = 2.3, label = TeX(paste0("$beta_{bc}$ = "," 0.25 ", " 95%CI: 0.08, 0.41**")), size=3, color = "pink4")
...
k4<-ggplot(data= dat_slopes)+
  theme_classic()+
  geom_point(aes(x=bold_I,y=bold_E), color ="darkolivegreen4", size = 3)+
  geom_segment(data =dat_slopes,
...
k5<-ggplot(data= dat_slopes)+
  theme_classic()+
  geom_point(aes(x=exp_I,y=exp_E), color ="darkolivegreen4", size = 3)+
  geom_segment(data =dat_slopes,
...
k6<-ggplot(data= dat_slopes)+
  theme_classic()+
  geom_point(aes(x=act_I,y=act_E), color ="darkolivegreen4", size = 3)+
  geom_segment(data =dat_slopes,
