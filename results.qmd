...
#Is the repeatability different from 5? #trouver papier avec seuil de valeur
...
#Average boldness
  p_value_bold_a<-pmcmc(R_bold*100, null = 10, twotail = FALSE)#not significative but really close
...
# See if syndromes differ in C and E
  ...
  pmcmc_b_act_C<-pmcmc(bsyn_b_act_C[,1] - bsyn_b_act_E[,1], twotail = TRUE)
   pmcmc_b_act_C<-pmcmc(bsyn_b_act_C[,1], twotail = TRUE)
   pmcmc_b_act_E<-pmcmc(bsyn_b_act_E[,1], twotail = TRUE)#no
...
#created dataset for all repeatability measures
  #Create data frame
dat_rep<-data.frame(
  "R"=c("Shyness","Exploration","Activity","Shyness","Exploration","Activity","Shyness","Exploration","Activity"),
  ...
  "Repeatability"=c(R_bold_mean,R_exp_mean,R_act_mean
                    ,R_C_bold,R_exp_C_mean,R_act_C,R_E_bold,R_exp_E_mean,R_act_E), 
  "Lower_CI" = c(R_Blb_bold,R_Blb_exp,R_Blb_act,R_Blb_bold_C,R_Blb_exp_C,R_Blb_act_C,R_Blb_bold_E,R_Blb_exp_E,R_Blb_act_E),
  "Higher_CI" = c(R_Bub_bold,R_Bub_exp,R_Bub_act,R_Bub_bold_C,R_Bub_exp_C,R_Bub_act_C,R_Bub_bold_E,R_Bub_exp_E,R_Bub_act_E))
...
dat_rep$Treatment = factor(dat_rep$Treatment, levels=c('Overall','Uninfected','Infected'))
...
We found evidence of behavioural syndromes between some traits depending on the treatment group (@fig-fig2). Activity and shyness were negatively and significantly correlated in the uninfected group (cor = `r b_act_C_mean`,95% CI = `r blb_bul_b_act_C`, $pMCMC_{\alpha = 0.05}$ = `r pmcmc_b_act_C`), but not in the infected group (cor = `r b_act_E_mean`,95% CI = `r blb_bul_b_act_E`, $pMCMC_{\alpha = 0.05}$ = `r pmcmc_b_act_E`). Activity and exploration were positively and significantly correlated in the uninfected group as well (cor = `r act_exp_C_mean`,95% CI = `r blb_bul_act_exp_C`, $pMCMC_{\alpha = 0.05}$ = `r pmcmc_act_exp_C`), but not in the infected group (cor = `r act_exp_E_mean`,95% CI = `r blb_bul_act_exp_E`, $pMCMC_{\alpha = 0.05}$ = `r pmcmc_act_exp_E`). Shyness and exploration were not significantly correlated in both treatment (cor = `r mean(bsyn_b_exp)`,95% CI = `r q_b_exp`). 


#### do we keep this? Do we need to look at if they are different if some are significant and other not?###
We did not observe among-individual variation to be higher in the uninfected vs experimentally infected group (@tbl-tab1). Repeatability did not differ between treatment for shyness ($pMCMC_{\alpha = 0.05}$ = `r p_value_bold`), exploration ($pMCMC_{\alpha = 0.05}$ = `r p_value_exp`) and activity ($pMCMC_{\alpha = 0.05}$ = `r p_value_act`). 

##Same thing here; do we keep this?##
The correlation did not differ between the uninfected and experimentally infected group for these syndromes (Shyness-Activity: $pMCMC_{\alpha = 0.05}$ =  `r pmcmc_b_act`; Shyness-Exploration: $pMCMC_{\alpha = 0.05}$ =  `r pmcmc_b_exp`; Activity-Exploration: $pMCMC_{\alpha = 0.05}$ =  `r pmcmc_act_exp`).

```{r}
#| label: fig-fig2
#| fig-cap: Correlation coefficients and their 95% confidence intervals for the three behaviours (shyness,exploration and activity) of pumpkinseed sunfish (Lepomis gibbosus) measured overall and within each group (uninfected and experimentally infected). Overall and uninfected: N= 72; Infected: N = 60.
#| out-width: 100%
#| fig-width: 7
#| fig-height: 8
...
bold_pl<-post_sd_pl[,grep("b_logboldness_z_dens", colnames(post_sd_pl))]
exp_pl<-post_sd_pl[,grep("b_exploration_z_dens", colnames(post_sd_pl))]
act_pl<-post_sd_pl[,grep("b_logactivity_z_dens", colnames(post_sd_pl))]
...
cor(all_data$dens_bs,all_data$dens_ces)
plot(all_data$dens_bs,all_data$dens_ces)
...
```{r}
#| label: fig-fig8
#| fig-cap: Change in black spots density and body condition for slope of change in activity
#| out-width: 100%
#| fig-width: 7
#| fig-height: 4
...
Behaviour is generally repeatable across trials and a meta anlysis found that the average repeatability for a wide variety of behaviours and taxa is 0.37 [Bell_2009-wg].

Exploration
Exploratory behaviour repeatability ranged from 0.27-0.66 for great tits bird [@Dingemanse_2002-wg]. A repeatability of 0.40 was found in pumpkinseed sunfish for exploration (Vic, in prep).A study found a repeatability of 0.47 for distance moved in cm for zebrafish and 0.35 for exploration [@Thomson_2020-wg].

Boldness 
A study on reef fish showed moderate repeatability for boldness (R = 0.38) for a short period in the lab and highly repeatable in the field (R= 0.69) [@White_2015-wg]

 boldness and exploration averaged (R) 0.300 and 0.313, respectively (https://academic.oup.com/beheco/article/26/3/900/235074?login=false#43508879)
 
Activity

Behavioural syndromes changed between the uninfected and infected group. 
Parasites could change the slope of a behavioural reaction norm without necessarily changing the average value of a behavioural trait or the variance in its expression (https://journals.biologists.com/jeb/article/216/1/18/11357/Parasite-manipulation-of-host-personality-and). Could making the response of the host completely independent from the context in which it is expressed. Parasites may have specific effects on a particular behavioural trait without affecting others, therefore decoupling traits and/or changing the way in which they correlate with each other. the vast majority of past studies, even when focusing on multiple host traits, have tended to only compare average (or median) trait values between parasitised and non-parasitised individuals, and not the relationships between different traits (Poulin, 2010).

parasites may have specific effects on a particular behavioural trait without affecting others, therefore decoupling traits and/or changing the way in which they correlate with each other (https://journals.biologists.com/jeb/article/216/1/18/11357/Parasite-manipulation-of-host-personality-and) -> in our case, activity is the only trait affected by parasite in our models. Act-exp and act-boldness are the one that are significantly correlated before but not after the infection. For repeatablity, activity changed quite a lot before and after the experimental infection. Parasire might affect activity but not the other traits, meaning that the behavioural syndrome changed and repeatability as well. Activity is decreased with parasite, as well as the correlation between activity-exploration and activity-boldness. 
Many trematodes are known to manipulate their host for their transmission, as they have a complex life cycle and pumpkinseed sunfish are intermediate host that need to be eaten to continue the parasite life cycle. If the fish are less active, they could be easily be more predated by piscivirous bird. 
Mean activity was not changed, but correlation and repeatability was. Sometimes, parasites can alter a trait without changing the mean average. 

Since parasites didn't seem to affect significantly exploration and boldness, we think that fish may have being habituated to the test. Indeed, on a long period of time, in our case 2 months, individuals can become use to the test. It is hard to measure exploration as the novel environmment might not be as novel as we want it to be for the fish when they do the test multiple times. Another recent study tried to measure boldness in pumpkinseed sunfish with a different test, but the trait was not repeatable as well. We know that pumpkinseed sunfish are, in general, bold and curious fish (Personal observations). If all the individuals tested are rather bold, repeatability could be smaller. Exploration and activitiy were repeatable in the uninfected state of the fish, but not boldness. It is possible that our test was not suited for this species. 
...
data_control <- read.table("./data_raw/12_fish.csv",header=T, sep=";")
...
rep3<-rpt(log(boldness) ~ ( 1 | ID_fish) + trial + ( 1 | cage), grname = "ID_fish", data = data_control, datatype = "Gaussian", 
    nboot = 1000, npermut = 0)
summary(rep3)
