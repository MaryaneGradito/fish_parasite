...
raw_int_exp_c<-(exp_int * sd_exploration) + mean_exploration
range(raw_int_exp_c) # Seems sensible


# ------------- REPEAT ABOVE FOR MEANS IN OTHER TREATMENTS: You use the dame mean_exploration and sd_exploration -------#
...
annotate("text",  x = 3, y = 4.1, label = TeX(paste0("$Contrast_{U - C}$ = -0.22, $pMCMC_{\\alpha = 0.05}$ = 0.381")), size =2.5)
 ...
 ggsave("./output/figures/fig2.png", plot = m, width = 13, height = 27, units = "cm")
...
#plot
  ...
  geom_point(mapping = aes(x = z_bc, y = z_log_activity), shape = 16,color = "#1f978b")+
 geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_act_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#1f978b", size=1) +
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#1f978b", size=0.8)+
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#1f978b", size=0.8)+
   theme_classic() +
    draw_image("./img/no_parasite.png", x= 2,y = 2, scale = 1)+
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition (g/cm)"), y = TeX("$Activity_{log_{2}}$"), title = "")+
      annotate("text", x = 0, y = 2.4, label = TeX(paste0("$beta_{bc}$ = ", " 0.21 ", " 95%CI: 0.04, 0.39**")), size =3, color = "black") 
...
x2<- ggplot(data=dat_parasite)+
  geom_point(mapping = aes(x = z_bc, y = z_log_activity), shape = 16,color = "#472a7a")+
 geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_act_bc), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#472a7a", size=1) +
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#472a7a", size=0.8)+
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#472a7a", size=0.8)+
   theme_classic() +
  draw_image("./img/parasite.png", x= 0.8,y = 1.2, scale = 1)+
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition (g/cm)"), y = TeX("$Activity_{log_{2}}$"), title = "")+
  annotate("text", x = -1, y = 1.7, label = TeX(paste0("$beta_{bc}$ = "," 0.25 ", " 95%CI: 0.09, 0.42**")), size=3, color = "black")
 ...
 geom_smooth(data = act_ces_dat, ggplot2::aes(x = z_ces, y = summ_act_ces), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#9E60AE", size=1) +
  ...
  draw_image("./img/blackspots.png", x = 2.8, y = -1.5, scale = 0.8)+
    ...
    theme(legend.position="none") + labs(x = TeX(""), y = TeX("$Activity_{log_{2}}$"), title = "")+
      annotate("text", x = 2, y = 2.2, label = TeX(paste0("$beta_{bs}$ = ", " -0.22 ", " 95%CI: -0.40, -0.04 **")), size =3, color = "#059687") +
  annotate("text", x = 2, y = 2, label = TeX(paste0("$beta_{ces}$ = "," 0.01 ", " 95%CI: -0.18, 0.19")), size=3, color = "#8F05B3")
