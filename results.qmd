...
geom_abline(intercept=-0.26, slope=-0.08, color ="darkgoldenrod", size = 1.2)+
   theme_classic() +
  draw_image("bs.png", x = 3, y = -1.5, scale = 1.2)+
    draw_image("cestode.png", x = 3, y = 0, scale = 1.2) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
...
geom_abline(intercept=0.24, slope=0.15, color ="darkgoldenrod", size = 1)+
   theme_classic() +
    ...
    annotate("text", x = -1.7, y = 0.7, label = TeX(paste0("Number of cestodes")), size =3.5, color = "aquamarine4", fontface = "bold") +
  annotate("text", x = -1.7, y = -0.2, label = TeX(paste0("Number of trematodes")), size =3.5, color = "darkgoldenrod4") +
...
n3<-ggplot(data=dat_parasite)+
 geom_point(mapping = aes(x = z_ces, y = log_activity), color = "aquamarine3")+
  geom_abline(intercept=-0.14, slope=0.0, color = "aquamarine3", size = 1.2)+
 geom_point(mapping = aes(x = z_bs, y = log_activity), color ="darkgoldenrod")+
geom_abline(intercept=-0.14, slope=-0.23, color ="darkgoldenrod", size = 1.2)+
   theme_classic() +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Parasite load"), y = TeX("$Activity_{log_{2}}$"), title = "")+
  annotate("text", x = -1.7, y = 0.5, label = TeX(paste0("Number of trematodes")), size =3.5, color = "darkgoldenrod4", fontface = "bold") +
  annotate("text", x = -1.7, y = -0.2, label = TeX(paste0("Number of cestodes")), size =3.5, color = "aquamarine4") +
...
#| fig-height: 8
  ...
  posterior <- as_draws_df(model6, vars = "^b")
  logact_pl <- data.frame(posterior[,grep("b_logactivity_z_pl", colnames(posterior))])
  logact_bc <- data.frame(posterior[,grep("b_logactivity_z_bc", colnames(posterior))])
  
mean_logact_pl <- apply(logact_pl,2, function(x) mean(x))
se_log_act_pl <- apply(logact_bc,2, function(x) sd(x))
test <- cbind(mean_logact_pl,se_log_act_pl)
