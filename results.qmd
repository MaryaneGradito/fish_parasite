...
#all_data2 <- read.table("./output/dat_bc.csv",header=T, sep=",")

#all_data2$trial <- as.factor(all_data2$trial )
#boxplot(all_data2$body_condition ~ all_data2$trial)
...
geom_violin(aes(), trim = FALSE) +
geom_jitter(aes(colour = trt),shape=16, position=position_jitter(0.1)) +  scale_color_manual(labels = c("Uninfected/Control","Infected","Control"), values = c("#9A6FA5", "#6FA5A4","#FAFA8B"))+
