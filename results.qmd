...
theme_classic() +
 ...
 # draw_image("./img/parasite.png", x= 0.5,y = 4, scale = 0.9)+
  #draw_image("./img/no_parasite.png", x= 1.5,y = 4, scale = 0.9)+
   ...
   #raw_image("./img/fleche_gauche.png", x= 1,y = 3.5, scale = 0.6)+
...
annotate("text",  x = 2.5, y = 3, label = TeX(paste0("$Contrast_{C - I}$ = 0.04,", " 95%CI = -0.43, 0.53")), size =4) + 
 ...
 ggsave("./output/figures/fig2.b.png", plot = m1, width = 15, height = 15, units = "cm")
 ggsave("./output/figures/fig2.ex.png", plot = m2, width = 15, height = 15, units = "cm")
 ggsave("./output/figures/fig2.act.png", plot = m3, width = 15, height = 15, units = "cm")
...
act_ces<-post_sd_2_p[,grep("b_zlogactivity_z_ces", colnames(post_sd_2_p))]
  ...
  #################################
# MODEL 12: how infection changed body condition
#################################
#Extract post sd from model 12.1
post_sd_bc<- as_draws_df(model_12.1, variable = "^b", regex = TRUE)

#extrait effect of parasite density on body condition
bc_ces<-post_sd_bc[,grep("b_z_ces", colnames(post_sd_bc))]
bc_bs<-post_sd_bc[,grep("b_z_bs2", colnames(post_sd_bc))]
bc_treatmentE<-post_sd_bc[,grep("b_treatmentE", colnames(post_sd_bc))]
bc_treatmentT<-post_sd_bc[,grep("b_treatmentT", colnames(post_sd_bc))]

  #mean and quantile for cestode density
  bc_ces_num<- as.numeric(unlist(bc_ces))
  bc_ces_mean <- mean(bc_ces_num)
  bc_Blb_ces <- round(quantile(bc_ces_num, c(0.025, 0.975))[1], 3)
  bc_Bub_ces <- round(quantile(bc_ces_num, c(0.025, 0.975))[2], 3)
  
  #mean and quantile for BS density
  bc_bs_num<- as.numeric(unlist(bc_bs))
  bc_bs_mean <- mean(bc_bs_num)
  bc_Blb_bs <- round(quantile(bc_bs_num, c(0.025, 0.975))[1], 3)
  bc_Bub_bs <- round(quantile(bc_bs_num, c(0.025, 0.975))[2], 3)
  
   #mean and quantile for treatment E 
  bc_E_num<- as.numeric(unlist(bc_treatmentE))
  bc_E_mean <- mean(bc_E_num)
  bc_Blb_E <- round(quantile(bc_E_num, c(0.025, 0.975))[1], 3)
  bc_Bub_E <- round(quantile(bc_E_num, c(0.025, 0.975))[2], 3)
  
     #mean and quantile for treatment T
  bc_T_num<- as.numeric(unlist(bc_treatmentT))
  bc_T_mean <- mean(bc_T_num)
  bc_Blb_T <- round(quantile(bc_T_num, c(0.025, 0.975))[1], 3)
  bc_Bub_T <- round(quantile(bc_T_num, c(0.025, 0.975))[2], 3)
  
  #################################
# MODEL 13: how co-infection impact body condition
#################################
#Extract post sd from model 13
post_sd_bc_13<- as_draws_df(model_13, variable = "^b", regex = TRUE)

#extrait effect of parasite density on body condition
bc_ces_13<-post_sd_bc_13[,grep("b_z_ces", colnames(post_sd_bc_13))]
bc_bs_13<-post_sd_bc_13[,grep("b_z_bs_2", colnames(post_sd_bc_13))]
bc_trial<-post_sd_bc_13[,grep("b_trial", colnames(post_sd_bc_13))]

  #mean and quantile for cestode density
  bc_ces_13_num<- as.numeric(unlist(bc_ces_13))
  bc_ces_mean_13 <- mean(bc_ces_13_num)
  bc_Blb_ces_13 <- round(quantile(bc_ces_13_num, c(0.025, 0.975))[1], 3)
  bc_Bub_ces_13 <- round(quantile(bc_ces_13_num, c(0.025, 0.975))[2], 3)
  
  #mean and quantile for BS density
  bc_bs_13_num<- as.numeric(unlist(bc_bs_13))
  bc_bs_13_mean <- mean(bc_bs_13_num)
  bc_Blb_bs_13 <- round(quantile(bc_bs_13_num, c(0.025, 0.975))[1], 3)
  bc_Bub_bs_13 <- round(quantile(bc_bs_13_num, c(0.025, 0.975))[2], 3)
  
   #mean and quantile for trial
  bc_trial_num<- as.numeric(unlist(bc_trial))
  bc_trial_mean <- mean(bc_trial_num)
  bc_Blb_trial <- round(quantile(bc_trial_num, c(0.025, 0.975))[1], 3)
  bc_Bub_trial <- round(quantile(bc_trial_num, c(0.025, 0.975))[2], 3)
...
#plot
  ...
  geom_point(mapping = aes(x = body_condition, y = log_activity), shape = 16,color = "#8A05A9")+
 geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = bc, y = raw_act_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#8A05A9", size=1) +
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = bc, y = raw_act_L95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#8A05A9", size=0.8)+
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = bc, y = raw_act_U95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#8A05A9", size=0.8)+
   theme_classic() +
    #draw_image("./img/no_parasite.png", x = 0.028, y = 2.4, scale = 50) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition (${g/cm^{3}}$)"), y = TeX("Distance swam$_{log_{2}}$ (cm)"), title = "")+
...
x2<- ggplot(data=dat_parasite)+
  geom_point(mapping = aes(x = body_condition, y = log_activity), shape = 16,color = "#26ABA5")+
 geom_smooth(data = act_bc_dat, ggplot2::aes(x = bc, y = raw_act_bc), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#26ABA5", size=1) +
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = bc, y = raw_act_L95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#26ABA5", size=0.8)+
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = bc, y = raw_act_U95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#26ABA5", size=0.8)+
   theme_classic() +
 draw_image("./img/parasite.png", x= -1,y = 1, scale = 100)+
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition (${g/cm^{3}}$)"), y = TeX("Distance swam$_{log_{2}}$ (cm)"), title = "")+
 annotate("text", x = 0.025, y = 1.7, label = TeX(paste0("$beta_{bc}$ = "," 0.26 ", " 95%CI = 0.09, 0.43**")), size=3, color = "black")

#################################
# model 13: how co-infection affect body condition
#################################
dat_parasite <- read.table("./output/dat_models_parasite.csv",header=T, sep=",")

 # First we want to create a design matrix.
newdat_13 <- data.frame(intercept = 1,
                        z_ces = seq(min(dat_parasite$z_ces), max(dat_parasite$z_ces), length.out = 100),
                        trial = NA,
                        z_bs_2 = 0)

  #body condition for cestode density
  pred_bc_ces_brms <- posterior_epred(model_13, re_formula = NA, newdata = newdat_13)
  bc_ces <- pred_bc_ces_brms[,,"zbc"]
  summ_bc_ces <- apply(pred_bc_ces_brms, 2, function(x) mean(x))
  summ_actL95_bc_ces <- apply(pred_bc_ces_brms, 2, function(x) quantile(x, 0.025))
  summ_actU95_bc_ces <- apply(pred_bc_ces_brms, 2, function(x) quantile(x, 0.975))
  
  
  #create dataframe
act_bc_c_dat <- cbind(newdat_13, summ_bc_ces, summ_actL95_bc_ces, summ_actU95_bc_ces)

#plot
   x3<- ggplot(data=dat_parasite)+
  geom_point(mapping = aes(x = dens_ces, y = body_condition), shape = 16,color = "#8A05A9")+
 geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_bc_ces), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#8A05A9", size=1) +
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#8A05A9", size=0.8)+
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "#8A05A9", size=0.8)+
   theme_classic() +
    #draw_image("./img/no_parasite.png", x = 0.028, y = 2.4, scale = 50) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition (${g/cm^{3}}$)"), y = TeX("Distance swam$_{log_{2}}$ (cm)"), title = "")+
...
ggsave("./output/figures/fig3.un.png", plot = x1, width = 12, height = 12, units = "cm")
ggsave("./output/figures/fig3.in.png", plot = x2, width = 12, height = 12, units = "cm")
...
#bold_pl_dat <- cbind(newdat5, summ_bold_pl, summ_boldL95_pl, summ_boldU95_pl)
 ...
 draw_image("./img/cestode.png", x = 24, y = -0.5, scale = 6) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Parasite density (no/g)"), y = TeX("Emergence time$_{log_{2}}$"), title = "") +
    annotate("text", x = 15, y = 1.6, label = TeX(paste0("$beta_{bs}$ = ", " -0.09, ", " 95%CI = -0.29, 0.10")), size =3, color = "#440154") + 
  annotate("text", x = 15, y = 1.8, label = TeX(paste0("$beta_{ces}$ = "," 0.11, ", " 95%CI = -0.06, 0.39")), size=3, color = "#21918c")+
     theme(axis.text=element_text(size=16),
        axis.title=element_text(size=16, face="bold"))
...
n2<-ggplot(data=dat_parasite)+
 ...
 draw_image("./img/blackspots.png", x = 20, y = -0.16, scale = 6)+
  draw_image("./img/cestode.png", x = 24, y = -0.22, scale = 6) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Parasite density (no/g)"), y = TeX("Surface covered (%)"), title = "") +
  annotate("text", x = 15, y = 0.45, label = TeX(paste0("$beta_{bs}$ = ", " 0.10, ", " 95%CI = 0.09, -0.07")), size =3, color = "#440154") + 
  annotate("text", x = 15, y = 0.47, label = TeX(paste0("$beta_{ces}$ = "," -0.11,  ", " 95%CI = -0.29, 0.07")), size=3, color = "#21918c")+ 
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=16, face="bold"))
...
geom_point(mapping = aes(x = dens_ces, y = log_activity),color = "#26ABA5")+
 geom_smooth(data = act_ces_dat, ggplot2::aes(x = ces, y = raw_act_ces), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "#26ABA5", size=1) +
  ...
  draw_image("./img/cestode.png", x = 24, y = -0.8, scale = 6) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Parasite density (no/g)"), y = TeX("Distance swam$_{log_{2}}$"), title = "") +
      annotate("text", x = 15, y = 1.6, label = TeX(paste0("$beta_{bs}$ = ", " -0.21, ", " 95%CI = -0.39, -0.03 **")), size =3, color = "#440154") +
 annotate("text", x = 15, y = 1.8, label = TeX(paste0("$beta_{ces}$ = "," 0.05, ", " 95%CI = -0.13, 0.24")), size=3, color = "#21918c")+
   theme(axis.text=element_text(size=16),
        axis.title=element_text(size=16, face="bold"))
...
ggsave("./output/figures/fig4.b.png", plot = n1, width = 12, height = 12, units = "cm")
ggsave("./output/figures/fig4.ex.png", plot = n2, width = 12, height = 12, units = "cm")
ggsave("./output/figures/fig4.act.png", plot = n3, width = 12, height = 12, units = "cm")
...
ggsave("./output/figures/fig_var.png", plot = v4, width = 17, height = 7, units = "cm")
...
```{r}
#| label: tbl-tab1
#| tbl-cap: Repeatability (R) estimates with 95% confidence intervals (CI) (i.e. did not overlap 0.10) for the three behaviours (emergence time, surface covered and distance swam) for overall (n = 72), uninfected (n = 72),  experimentally infected (n = 60) and control fish (n = 12). 

tab1 <- data.frame(Trait = c("Log emergence time (sec)", "Surface covered (%)","Log distance swam (cm)"), 
"Overall" = c(R_bold_mean, R_exp_mean, R_act_mean),
"95% CI (O)" = c("[0.007, 0.362]","[0.019, 0.437]","[0.095, 0.497]**"),
"Uninfected" = c(mean(R_C_boldness[,1]), mean(R_C_exploration[,1]), mean(R_C_activity[,1])), 
"95% CI (U)" = c("[0.010, 0.366]","[0.106, 0.468]**","[0.160, 0.516]**"),
"Infected" = c(mean(R_E_boldness[,1]), mean(R_E_exploration[,1]), mean(R_E_activity[,1])),
"95% CI (I)" = c("[0.003, 0.388]","[0.004, 0.433]","[0.047, 0.515]"), 
"Control" = c(R_T_bold, R_T_exp,R_T_act),
"95% CI (C)" = c("[0, 0.586]","[0, 0.493]","[0.001, 0.733]"),check.names = FALSE)
   ...
   gtsave(tab1, filename = "tab1.html")
...
tab2 <- data.frame("Raw variables" = c("Emergence time (sec)", "Surface covered (%)","Distance swam (cm)","Body condition (mass/length^3)", "Parasite density (no/g)", "Black spot density (no/g)","Cestode density (no/g)"), 
  ...
  fmt_number(columns = 5, decimals = 3)
   ...
   gtsave(tab2, filename = "tab2.html")

```

```{r}
tab3 <- data.frame("Models" = c("Model 1", "Model 2","Model 3","Model 4","Model 5","Model 6"),
"Formula" = c("boldness/activity/exploration ~ 1 + treatment + (-1 + treatment |q| ID_fish) + (1|cage), sigma ~ -1 + treatment (uninfected; infected; control)","boldness/activity/exploration ~ 1 + treatment + (-1 + treatment |q| ID_fish) + (1|cage), sigma ~ -1 + treatment (uninfected;infected)", "boldness/activity/exploration ~ 1 + treatment + (treatment |q| ID_fish) + (1 | cage), sigma ~ -1 + treatment (uninfected; infected)","body condition ~ 1 + cestode density + black spot density + treatment + (1 | ID_fish) + (1 | cage) (uninfected; infected; control)", "boldness/activity/exploration ~ 1 + body condition + (1|ID_fish) + (1|cage)",
"boldness/activity/exploration ~ 1 + cestode density + black spot density + body condition + (1|ID_fish) + (1|cage)"),
...
tab3<-gt(tab3)  %>% 
  tab_style(style = list(cell_text(weight = "bold")),
    ...
    tab_style(style = cell_borders(sides = "bottom", weight = px(2)),
      ...
      tab_style(style = cell_borders(sides = "bottom", weight = px(1)),
         locations =  cells_body(rows = 1))%>%
      tab_style(style = cell_borders(sides = "bottom", weight = px(0.5)),
  ...
  gtsave(tab3, filename = "tab3.html")


```

```{r}
tab4 <- data.frame("Variable" = c("Intercept","Cestode density","Black spot density","Trial"),
                   "Estimate" = c("0.32±0.14","-0.30±0.10","-0.15±0.10","-0.63±0.13"),
                    "CI95" = c("[0.05, 0.58]","[-0.49, -0.10]**","[ -0.35, 0.04]","[-0.90, -0.37]"),
                  "Rhat" = c(1.00,1.00,1.00,1.00))
...
tab4<-gt(tab4)  %>% 
  ...
  gtsave(tab4, filename = "tab4.html")
```

```{r}
tab5 <- data.frame("Variable" = c("Intercept","Cestode density","Black spot density","Infected","Control"),
                   "Estimate" = c("-0.15±0.14","-0.18±0.07","-0.13±0.08","0.30±0.17","0.27±0.26"),
                    "CI95" = c("[-0.44, 0.14]","[-0.33, -0.04]**","[-0.28, 0.02]","[-0.04, 0.64]","[-0.23, 0.77]"),
...
tab5<-gt(tab5)  %>% 
  ...
  gtsave(tab5, filename = "tab5.html")

```

```{r}
tab6 <- data.frame("Response variable" = c("Boldness", "Exploration","Activity","Boldness", "Exploration","Activity","Boldness", "Exploration","Activity"),
"Interaction" = c("Black spot*cestode", "Black spot*body condition","Cestode*body condition","Black spot*cestode", "Black spot*body condition","Cestode*body condition","Black spot*cestode", "Black spot*body condition","Cestode*body condition"),
"Estimate" = c("0.01±0.11","0.17±0.10","0.02±0.11","0.06±0.11","-0.03±0.09","-0.11±0.10","0.11±0.14","-0.10±0.12","0.07±0.13"),
"95IC"= c("[-0.20,0.24]","[-0.03,0.37]","[-0.20, 0.24]","[-0.15,0.28]","[-0.21,0.16]","[-0.31,0.08]","[-0.16,0.38]","[-0.34,0.13]","[-0.18,0.32]"),
