...
All statistical analyses were performed in R (vers. `r paste0(R.Version()$major, ".", R.Version()$minor)`). Adjusted fish mass was calculated as the fish mass minus parasite mass (Lagrue and Poulin, 2015). Parasite mass was estimated with the mean mass of 20 parasites (Vic, in prep). For bass tapeworm, adult average mass was 0.003g and the larval form was 0.0008g. We decided to exclude nematodes since the mass varied greatly between individuals, and was rarely found in our fish. Blackspot mass was considered too small to be subtracted from total mass ($10^{-7}g$). Body condition was calculated with the Fulton index ($mass/length^3$) (Jakob, 1996). We had 4 measures of body condition, one for each trial. We used the adjusted fish mass for the 2 measures of body condition after the experimental infection (we assume there was no parasites before the experimental infection). We used standard length of the fish in centimeters. We calculated parasite load by adding the total number of blackspots post-infection with the total number of cestodes found alive (adult and larvae form). We could not determine if larvae form was alive or not visually, but we assumed it can have an effect if the larva is developping in the fish. 

Exploration was normally distributed while activity and boldness were log-transformed to better approximate normality. All variable were z-scaled (i.e., $\left(x - \bar{x} \right) / \sigma$) before fitting the models to ease model interpretation and improve paramater estimation. We fitted our models within a Bayesian statistical framework using the *brms* [vers. `r utils::packageVersion("brms")` [@Burkner2017-wg] package and *rstan* [vers. `r utils::packageVersion("rstan")`] [@noauthor_2021-az].  We ran four MCMC chains each with 6000 iterations, sampling every iteration. We discareded the first 2000 iterations as a warmup. We visually inspected chains to ensure that they were mixing well and that chains had converged (i.e., Rhat < 1.02), and ensured that we have an effective sample size >1000 for all parameters. 

To test if traits were repeatabile (i.e., we had personality) and if they formed behavioural syndromes, we built a multi-responses model (n = 264, observations) with each of the three traits as response variables. Treatment (uninfected vs infected) was included as a fixed effect for each trait because we were interested in whether infection changed fish behvaiour. Fish ID and cage number were included as random effects. Model 1 had tank effect as a fixed factor compared to Model 2. We compared the models using the *loo* package [vers. `r utils::packageVersion("loo")`]. Model 2 was better fitted than model 1 and we decided not to include tank effect in our models.
...
```{r}
#| label: tbl-tab1
#| tbl-cap: Repeatbility for uninfected and infected group
#| 
...
pp<-plot_grid(
  ...
  labels = "AUTO", ncol = 1)
pp
ggsave("fig1.png", plot = pp, width = 28, height = 30, units = "cm")
```

#### *Boldness and exploration responses differ between uninfected and infected group*

Mean boldness and exploration responses in the population were significantly different between the uninfected and experimentally group, but not for activity (@fig-fig2).

```{r}
#| label: fig-fig2
#| fig-cap: Effect of treatment on trait responses
#| out-width: 100%
#| fig-width: 8
#| fig-height: 5
...
scale_color_manual(labels = c("Uninfected", "Infected"), values = c("darkgoldenrod", "aquamarine3"))+
...
annotate("text",  x = 1.5, y = 4, label = TeX(paste0("$pMCMC_{\\alpha = 0.05}$ = 0.0666")), size =2.5)
 ...
 m<-plot_grid(
  ...
  labels = "AUTO", ncol = 3)
 m
 ggsave("fig2.png", plot = m, width = 28, height = 13, units = "cm")
...
```{r}
#| label: fig-fig3
#| fig-cap: Effect of body condition on traits
#| out-width: 100%
#| fig-width: 8
#| fig-height: 5
...
#plot
  ...
  geom_point(mapping = aes(x = z_bc, y = log_activity), shape = 21,color = "mediumorchid4")+
 geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_act_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "mediumorchid4", size=1) +
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "mediumorchid4", size=0.8)+
  geom_smooth(data = act_bc_c_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc_c), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "mediumorchid4", size=0.8)+
   theme_classic() +
    draw_image("no_parasite.png", x= 2,y = 2.1, scale = 0.9)+
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition $(g/cm^{3})$"), y = TeX("$Activity_{log_{2}}$ (cm)"), title = "")+
      annotate("text", x = 0, y = 2.6, label = TeX(paste0("$beta_{bc}$ = ", " 0.21 ", " 95%CI: 0.04, 0.39**")), size =3, color = "mediumorchid4") 
...
x2<- ggplot(data=dat_parasite)+
  geom_point(mapping = aes(x = z_bc, y = log_activity), shape = 21,color = "mediumorchid4")+
 geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_act_bc), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "mediumorchid4", size=1) +
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_actL95_bc), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "mediumorchid4", size=0.8)+
  geom_smooth(data = act_bc_dat, ggplot2::aes(x = z_bc, y = summ_actU95_bc), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "mediumorchid4", size=0.8)+
    geom_point(mapping = aes(x = z_pl, y = log_activity), shape = 21,color = "darkgoldenrod")+
 geom_smooth(data = act_pl_dat, ggplot2::aes(x = z_pl, y = summ_act_pl), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "darkgoldenrod", size=1) +
  geom_smooth(data = act_pl_dat, ggplot2::aes(x = z_pl, y = summ_actL95_pl), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
  geom_smooth(data = act_pl_dat, ggplot2::aes(x = z_pl, y = summ_actU95_pl), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
   theme_classic() +
  draw_image("parasite.png", x= 2,y = 2, scale = 1.2)+
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Body condition $(g/cm^{3})$"), y = TeX("$Activity_{log_{2}}$ (cm)"), title = "")+
  annotate("text", x = 3, y = -1.5, label = TeX(paste0("Parasite load")), size =3.5, color = "darkgoldenrod4", fontface = "bold") +
  annotate("text", x = -2, y = 0, label = TeX(paste0("Body condition")), size =3.5, color = "mediumorchid4") +
      annotate("text", x = 0, y = 2.5, label = TeX(paste0("$beta_{pl}$ = ", " -0.18 ", " 95%CI: -0.37, 0.01")), size =3, color = "darkgoldenrod4") + 
  annotate("text", x = 0, y = 2.3, label = TeX(paste0("$beta_{bc}$ = "," 0.25 ", " 95%CI: 0.08, 0.41**")), size=3, color = "mediumorchid4")
 ...
 x<- plot_grid(
  ...
  x    
    ggsave("fig3.png", plot = x, width = 28, height = 16, units = "cm")
...
```{r}
#| label: fig-fig4
#| fig-cap: Parasite species effect on traits
#| out-width: 100%
#| fig-width: 4
#| fig-height: 10
...
boldness_bs_dat <- cbind(newdat2, summ_bold_bs, summ_boldL95_bs, summ_boldU95_bs)
 ...
 # pred_ces <- predict(model7, re_formula = NA, newdata = newdat)
  ...
  geom_point(mapping = aes(x = z_ces, y = log_boldness),shape = 21, color = "aquamarine3")+
  geom_smooth(data = boldness_ces_dat, ggplot2::aes(x = z_ces, y = summ_bold_ces), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "aquamarine3", size=1) +
  geom_smooth(data = boldness_ces_dat, ggplot2::aes(x = z_ces, y = summ_boldL95_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
  geom_smooth(data = boldness_ces_dat, ggplot2::aes(x = z_ces, y = summ_boldU95_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8) + 
    geom_point(mapping = aes(x = z_bs, y = log_boldness), shape = 21, color = "darkgoldenrod")+
  geom_smooth(data = boldness_bs_dat, ggplot2::aes(x = z_bs, y = summ_bold_bs), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "darkgoldenrod", size=1) +
  geom_smooth(data = boldness_bs_dat, ggplot2::aes(x = z_bs, y = summ_boldL95_bs), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
  geom_smooth(data = boldness_bs_dat, ggplot2::aes(x = z_bs, y = summ_boldU95_bs), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
   theme_classic() +
  draw_image("blackspots.png", x = 3, y = -1.2, scale = 0.8)+
    draw_image("cestodes.png", x = 3, y = -0.3, scale = 0.8) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX(""), y = TeX("$Boldness_{log_{2}}$ (sec)"), title = "") +
      annotate("text", x = 1.5, y = 3.2, label = TeX(paste0("$beta_{bs}$ = ", " -0.08 ", " 95%CI: -0.28, 0.12")), size =3, color = "darkgoldenrod4") + 
  annotate("text", x = 1.5, y = 3, label = TeX(paste0("$beta_{ces}$ = "," 0.11 ", " 95%CI: -0.10,0.31")), size=3, color = "aquamarine4")
...
n2<-ggplot(data=dat_parasite)+
  geom_point(mapping = aes(x = z_ces, y = exploration), shape = 21, color = "aquamarine3")+
 geom_smooth(data = exp_ces_dat, ggplot2::aes(x = z_ces, y = summ_exp_ces), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "aquamarine3", size=1) +
  geom_smooth(data = exp_ces_dat, ggplot2::aes(x = z_ces, y = summ_expL95_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
  geom_smooth(data = exp_ces_dat, ggplot2::aes(x = z_ces, y = summ_expU95_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
  geom_point(mapping = aes(x = z_bs, y = exploration), shape = 21, color = "darkgoldenrod") +
   geom_smooth(data = exp_bs_dat, ggplot2::aes(x = z_bs, y = summ_exp_bs), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "darkgoldenrod", size=1) +
  geom_smooth(data = exp_bs_dat, ggplot2::aes(x = z_bs, y = summ_expL95_bs), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
  geom_smooth(data = exp_bs_dat, ggplot2::aes(x = z_bs, y = summ_expU95_bs), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
   theme_classic() +
  draw_image("blackspots.png", x = 2.8, y = 0.2, scale = 0.8)+
    draw_image("cestodes.png", x = 2.8, y = -0.7, scale = 0.8) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX("Parasite load (no)"), y = TeX("Exploration (%)"), title = "") +
      annotate("text", x = 1.5, y = 3.2, label = TeX(paste0("$beta_{bs}$ = ", " 0.15 ", " 95%CI: -0.03, 0.32")), size =3, color = "darkgoldenrod4") + 
  annotate("text", x = 1.5, y = 3, label = TeX(paste0("$beta_{ces}$ = "," -0.10 ", " 95%CI: -0.27, 0.08")), size=3, color = "aquamarine4")

  #activity for cestode
    pred_ces_brms <- posterior_epred(model7, re_formula = NA, newdata = newdat)
...
geom_point(mapping = aes(x = z_ces, y = log_activity), shape = 21,color = "aquamarine3")+
 geom_smooth(data = act_ces_dat, ggplot2::aes(x = z_ces, y = summ_act_ces), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "aquamarine3", size=1) +
  geom_smooth(data = act_ces_dat, ggplot2::aes(x = z_ces, y = summ_actL95_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
  geom_smooth(data = act_ces_dat, ggplot2::aes(x = z_ces, y = summ_actU95_ces), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=0.8)+
  geom_point(mapping = aes(x = z_bs, y = log_activity),shape = 21, color = "darkgoldenrod") +
   geom_smooth(data = exp_bs_dat, ggplot2::aes(x = z_bs, y = summ_act_bs), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "darkgoldenrod", size=1) +
  geom_smooth(data = exp_bs_dat, ggplot2::aes(x = z_bs, y = summ_actL95_bs), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
  geom_smooth(data = exp_bs_dat, ggplot2::aes(x = z_bs, y = summ_actU95_bs), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=0.8)+
   theme_classic() +
  draw_image("blackspots.png", x = 2.8, y = -1.4, scale = 0.8)+
    draw_image("cestodes.png", x = 2.8, y = -0.6, scale = 0.8) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(legend.position="none") + labs(x = TeX(""), y = TeX("$Activity_{log_{2}} (cm)$"), title = "")+
...
ggsave("fig4.png", plot = p, width = 40, height = 15, units = "cm")
...
#### *Individuals changes in traits across treatment*
...
k6<-ggplot(data= dat_slopes)+
 ...
 w<-plot_grid(
  ...
  labels = "AUTO", ncol = 1)
 w
# ggsave("fig5.png", plot = w, width = 30, height = 20, units = "cm")
