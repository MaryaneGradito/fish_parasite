...
The average repeatability across trial was `r mean(R_boldness[,1])` (95% CI: `r R_Blb_bold` to `r R_Bub_bold`) for boldness; `r mean(R_explore[,1])`(95% CI: `r R_Blb_exp` to `r R_Bub_exp`) for exploration; and `r mean(R_activity[,1])`(95% CI: `r R_Blb_act` to `r R_Bub_act`) for activity. Repeatability did not differ between treatment for boldness (p-value = `r p_value_bold`), exploration (p-value: `r p_value_exp`) and activity (p-value:`r p_value_act`).To estimate if repeatability was different from 0, we choose 0.05 as a thresold since the values can't be 0 in the distribution. Average repeatability was not significantly different from 0.05 for boldness (p-value = `r p_value_bold_a`), exploration (p-value = `r p_value_exp_a`) and activity (p-value = `r p_value_act_a`)).
