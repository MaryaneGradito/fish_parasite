...
# Lets create the predictions with the posterior for each variable. Here, we want to vary ces, but keep the other varibles at the mean

  # First we want to create a design matrix. 
  newdat <- data.frame(intrecept = 1, z_ces = seq(min(dat_parasite$z_ces), max(dat_parasite$z_ces), length.out = 100),
                        z_bs = 0,
                        z_bc = 0)
  ...
  # Make the predcitions. Here, each column is now a different combination of parameters, i.e., 1 MCMC iteration), and each row is the values you want to predcit, only z_ces varies
...
n2<-ggplot(data=dat_parasite)+
 geom_point(mapping = aes(x = z_ces, y = exploration),shape = 19, color = "aquamarine3")+
  geom_abline(intercept=0.24, slope=-0.10, color = "aquamarine3", size = 1)+
 geom_point(mapping = aes(x = z_bs, y = exploration), shape = 18, color ="darkgoldenrod")+
geom_abline(intercept=0.24, slope=0.15, color ="darkgoldenrod", size = 1)+
   theme_classic() +
