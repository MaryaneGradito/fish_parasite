...
############################
  ...
  R2_m <- bayes_R2(model3, re.form = NA) #marginal
  R2_c <- bayes_R2(model3) #conditional
...
p1<-
  ...
  data    = dat_text,
  mapping = aes(x = -1, y = 3, label = label, group = NULL),size = 3)
  ...
  label = c("rb = 0.70, 95%CI =  0.37, 0.93\
  ...
  geom_line(mapping = aes(x = log_activity, y = exploration, color = factor(ID_fish) )) +
    facet_wrap(~ treatment, labeller = as_labeller(plot_names)) +
    theme(strip.background = element_rect(fill = "white", colour = "white")) +
    theme(axis.title = element_text(),
         legend.position="none" ) + labs(x = TeX("$Activity_{log_{2}}$"), y = "Exploration") +
  geom_text(
  data    = dat_text3,
  mapping = aes(x = -1, y = 2.5, label = label, group = NULL),size = 3)
