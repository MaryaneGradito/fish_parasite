...
# Lets create the predictions with the posterior for each variable. Here, we want to vary ces, but keep the other variables at the mean
  ...
  geom_smooth(data = boldness_ces_dat, ggplot2::aes(x = z_ces, y = logboldU95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "aquamarine3", size=1.2)+
    geom_point(mapping = aes(x = z_bs, y = log_boldness), color = "darkgoldenrod")+
  geom_smooth(data = boldness_bs_dat, ggplot2::aes(x = z_bs, y = logbold), method =  "loess", formula = y~x, se = FALSE,lty = "solid", color = "darkgoldenrod", size=1.2) +
  geom_smooth(data = boldness_bs_dat, ggplot2::aes(x = z_bs, y = logbold_L95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=1.2)+
  geom_smooth(data = boldness_bs_dat, ggplot2::aes(x = z_bs, y = logboldU95), method =  "loess", formula = y~x, se = FALSE,lty = "dashed", color = "darkgoldenrod", size=1.2)+
   theme_classic() +
