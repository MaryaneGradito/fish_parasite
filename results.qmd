...
```{r, packages, warning=FALSE, include=FALSE}
    ...
    source("./R/func.R")
    pacman::p_load(tidyverse, brms, glmmTMB, flextable)
    options(digits = 3)
```
...
The average repeatbility of each traits across trial were `r round(R_exp_mean), 2)` (95% CI: `r R_Blb_bold` to `r R_Bub_bold`) for boldness;  `r round(R_exp_mean), 2)`(95% CI: `r R_Blb_exp` to `r R_Bub_exp`) for exploration; and  `r round(R_act_mean), 2)`(95% CI: `r R_Blb_act` to `r R_Bub_act`) for activity (tab1). 
