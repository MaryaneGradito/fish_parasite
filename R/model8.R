...
boldness_8 <- bf(z_log_boldness ~ 1 + z_ces + z_bc + z_ces*z_bc + (1 | ID_fish) + (1 | cage)) + gaussian()
activity_8 <- bf(z_log_activity ~ 1 + z_ces + z_bc + z_ces*z_bc + (1 | ID_fish) + (1 | cage)) + gaussian()
 explore_8 <- bf(z_exploration  ~ 1 + z_ces + z_bc + z_ces*z_bc + (1 | ID_fish) + (1 | cage)) + gaussian()


model_8_2 <- brms::brm(boldness_8 + activity_8 + explore_8 + set_rescor(TRUE), 
                     data = dat_parasite, iter = 6000, warmup = 2000, chains = 4, cores = 4, 
                     save_pars = save_pars(all = TRUE), file = "./output/models/model_8_2", file_refit = "on_change",
