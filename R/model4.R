...
### Model 4: experimental group
  ...
  boldness_4 <- bf(log_boldness ~ 1 + z_dens + (1 | ID_fish) + (1 | cage)) + gaussian()
  activity_4 <- bf(log_activity ~ 1 + z_dens + (1 | ID_fish) + (1 | cage)) + gaussian()
  explore_4 <- bf(exploration ~ 1 + z_dens  + (1 | ID_fish) + (1 | cage)) + gaussian()
